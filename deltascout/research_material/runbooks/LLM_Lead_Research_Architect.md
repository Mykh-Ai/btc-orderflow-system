...
Current repository state contains two different daily market-data contours relevant to research:

- `data/archive/feed/YYYY-MM-DD.csv` - local Aggregator archive contour written by the same repo that writes runtime `aggregated.csv`
- `/opt/aitrader/feed/YYYY-MM-DD.csv` - separate external research contour used by current offline/analyzer workflows

Treat these as two different minute contours in the current repository state.

For DeltaScout event-linked research, the analyst must state which contour is being used for each claim.

- Use `data/archive/feed/YYYY-MM-DD.csv` when the question is specifically about the local runtime writer chain behind `aggregated.csv` and `PEAK_EMIT`.
- Use `/opt/aitrader/feed/YYYY-MM-DD.csv` when the question is specifically about the current offline/analyzer workflow or enriched minute context.
- If both are used in the same workflow, event-source fields and enrichment-derived fields need to stay explicitly separated and any discrepancies need to be called out rather than normalized away.

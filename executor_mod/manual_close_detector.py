...
def tick(
    ...
    guard_quote = abs(quote_delta) < quote_eps

    side = str(pos.get("side") or "").upper()
    if side == "LONG":
        guard = guard_base and guard_quote and (not has_debt if trade_mode == "margin" else True)
    elif side == "SHORT":
        guard = (not has_debt) and guard_base and guard_quote
    else:
        guard = False
